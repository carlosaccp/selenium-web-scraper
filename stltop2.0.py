# Created by carlosaccp all rights reserved. The following code was written in MacOS Catalina 10.15.1
...
# Define the options for when the chromedriver opens, which for me is set the window size to my monitor's size 
...
# For context, I am interested in scraping all of the designers listed in STLTop's names and patron numbers, and there are itemppage designers lister per page
...
pgcounter = 1
# Create a itemppage variable which represents how many items are displayed in a given page
itemppage = 9

#Create a for loop that loops from when number == 1 to when number == elements
for number in range (1 , elements+1):
    # Redefine the counter variable as number % itemppage. This means that we will know what item of the page the program is scraping as there are itemppage items per page.
    counter = number % itemppage
    # If counter != 0 <==> if the last element of the page hasn't been reached and pgcounter != math.floor(elements/(itemppage+1)) <==> we are not on the last page, so
    # we need to scrape itemppage times, not any less as on the last page there is a itemppage-1/itemppage chance that there are < itemppage elements, so we need to find a way to find how many
    # elements are on the last page to only scrape said elements, not more as that leads to error
    if (counter != 0) and pgcounter != math.floor(elements/(itemppage+1)):
    ...
    # Else if counter == 0 <==> we are on the itemppage-th element of the page <==> we need to go onto the next page after scraping this element. Moreover
    # pgcounter != math.floor(elements/(itemppage+1)) <==> we are not on the last page so we don't need to worry about having < itemppage elements in the page
    elif (counter == 0) and pgcounter != math.floor(elements/(itemppage+1)):
            # Locate the designer's name, same as before except that this time as we are on the itemppage-th element we don't need to format the counter 
            # as counter == itemppgage always
            name = (driver.find_element_by_xpath('/html/body/section[2]/div/div[4]/div[{}]/div[2]/div/div[1]/div/div/h5'.format(itemppage)).text
            # Locate the designers' patrons with the same twist as the previous line, no formatting this time
            patrons = [int(s) for s in (driver.find_element_by_xpath('/html/body/section[2]/div/div[4]/div[{}]/div[2]/div/div[1]/div/div/h5'.format(itemppage)).text.split() if s.isdigit()][0]
    ...
    # Else if pgcounter == math.floor((elements/itemppage) + 1) <==> we are on the last page
    elif pgcounter == math.floor((elements/itemppage) + 1):
